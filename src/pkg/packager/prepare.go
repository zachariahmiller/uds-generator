...
import (
	...
	"github.com/defenseunicorns/uds-generator/src/internal/packager/helm"
	"github.com/defenseunicorns/uds-generator/src/internal/packager/kustomize"
	"github.com/defenseunicorns/uds-generator/src/internal/packager/template"
	"github.com/defenseunicorns/uds-generator/src/pkg/packager/creator"
	"github.com/defenseunicorns/uds-generator/src/pkg/packager/variables"
...
func (p *Packager) FindImages() (imgMap map[string][]string, err error) {
	fmt.Println("***** ewyles find images")
	cwd, err := os.Getwd()
	if err != nil {
		return nil, err
	}
...
// TODO ewyles this is an absolutely hacked copy of FindImages that I'm using
// to rip stuff out and get down to the minimum stuff needed to template the helm chart
// it causes an error at the moment because it's cramming the templated chart into
// the image map (key = chart name) and then it doesn't parse as an image.
// the function needs to return a map of string to string and forget all the image stuff
// happening downstream (marked with other todo)
func (p *Packager) GetManifests() (imgMap map[string][]string, err error) {
	fmt.Println("***** ewyles GetManifests")
	cwd, err := os.Getwd()
	if err != nil {
		return nil, err
	}
...
func (p *Packager) findImages() (imgMap map[string][]string, err error) {
	...
	for _, component := range p.cfg.Pkg.Components {
		...
		for _, chart := range component.Charts {
			...
			// TODO ewyles -- forget the rest of this and get chartTemplate into a simpler
			// return map
			imagesMap[component.Name] = append(imagesMap[component.Name], chartTemplate)
